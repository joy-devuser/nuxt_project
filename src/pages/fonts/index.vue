...
const {data: _posts, status:_status, error:_error} = await useFetch<FontPost[]>(`${langApi}/fonts?context=embed`)
  if (_error.value) {
      ...
      <div class="contentsContainer u_d_fl _rs">
        ...
        <div class="fontListContainer  u_fx1">
          <ul v-if="_posts" class="fontList">
            <li :class="`fontList-item font_${font.id}`" v-for="font in _posts.filter(_p => fonts_list_arr.includes(_p.id))" :key="font.id" >
          <div class="image">
